...
# Undertale风格对话框类
# ================================
class UndertaleDialogueBox:
    def __init__(self, text, speaker=None, color=None, has_heart=True):
        self.full_text = text
        self.displayed_text = ""
        self.text_index = 0
        self.speaker = speaker
        self.box_color = color or DIALOGUE_CONFIG['box_color']
        ...
        self.box_width = min(SCREEN_WIDTH - 100, 700)
        self.box_height = 120
        self.box_x = (SCREEN_WIDTH - self.box_width) // 2
        self.box_y = SCREEN_HEIGHT - 180

    def update(self, current_time):
        if self.complete:
            if self.has_heart:
                self.heart_x = (self.heart_x + 0.5) % 20
        ...
        if current_time - self.last_char_time >= self.char_delay:
            if self.text_index < len(self.full_text):
                char = self.full_text[self.text_index]
                if char == ' ' and self.text_index > 0 and self.full_text[self.text_index - 1] == ' ':
                    self.displayed_text += ' '
                else:
    ...
    def draw(self):
        border_color = COLORS['undertale_white']
        text_color = COLORS['undertale_white']

        pygame.draw.rect(screen, self.box_color, (self.box_x, self.box_y, self.box_width, self.box_height))

        pygame.draw.rect(screen, border_color, (self.box_x, self.box_y, self.box_width, self.box_height), 4)

        for i in range(4):
            corner_offset = i * 10
            if i == 0:
                corner_x, corner_y = self.box_x, self.box_y
            elif i == 1:
                corner_x, corner_y = self.box_x + self.box_width, self.box_y
            elif i == 2:
                corner_x, corner_y = self.box_x + self.box_width, self.box_y + self.box_height
            else:
                corner_x, corner_y = self.box_x, self.box_y + self.box_height

            pygame.draw.circle(screen, border_color, (corner_x, corner_y), DIALOGUE_CONFIG['corner_size'], 4)

        if self.speaker:
            name_surface = font.render(self.speaker, True, COLORS['undertale_yellow'])
            name_rect = name_surface.get_rect(center=(self.box_x + 80, self.box_y - 15))
            pygame.draw.rect(screen, self.box_color, (name_rect.x - 10, name_rect.y - 2, name_rect.width + 20, name_rect.height + 4))
            screen.blit(name_surface, name_rect)
            pygame.draw.rect(screen, border_color, (name_rect.x - 10, name_rect.y - 2, name_rect.width + 20, name_rect.height + 4), 2)

        text_x = self.box_x + DIALOGUE_CONFIG['box_padding']
        text_y = self.box_y + DIALOGUE_CONFIG['box_padding']
        max_width = self.box_width - DIALOGUE_CONFIG['box_padding'] * 2
        ...
        if self.complete and self.has_heart:
            heart_y = self.box_y + self.box_height - 35
            heart_x = self.box_x + 20 + int(self.heart_x)

            pygame.draw.polygon(screen, COLORS['undertale_heart_color'], [
                (heart_x, heart_y - 10),
                (heart_x - 10, heart_y),
                (heart_x, heart_y + 8),
                (heart_x + 10, heart_y)
            ...
            heart_y2 = self.box_y + self.box_height - 35
            pygame.draw.polygon(screen, COLORS['undertale_heart_color'], [
                (heart_x - 10, heart_y2 - 10),
                (heart_x - 20, heart_y2),
                (heart_x - 10, heart_y2 + 8),
                (heart_x, heart_y2)
            ])
    ...
    def handle_input(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key in [pygame.K_SPACE, pygame.K_RETURN, pygame.K_z]:
                ...
                self.char_delay = max(10, self.char_delay - 10)
...
class IntroSequence:
    ...
    def draw_intro_text(self):
        ...
        if speaker:
            ...
            pygame.draw.rect(screen, (0, 0, 0), (name_rect.x - 10, name_rect.y - 2, name_rect.width + 20, name_rect.height + 4))
            screen.blit(name_surface, name_rect)
            pygame.draw.rect(screen, color, (name_rect.x - 10, name_rect.y - 2, name_rect.width + 20, name_rect.height + 4), 2)
...
class StaticDeadBush:
    ...
    def draw(self):
        pygame.draw.rect(screen, COLORS['dead_tree'],
                        (self.x - self.trunk_width//2, self.y, self.trunk_width, self.size * 0.3))
        ...
        for i, branch in enumerate(self.branches):
            ...
            for offset in range(-branch['thickness']//2, branch['thickness']//2 + 1):
                pygame.draw.line(screen, (color_r, color_g, color_b),
                              (start_x + offset, start_y), (mid_x + offset, mid_y), 2)
                pygame.draw.line(screen, (color_r, color_g, color_b),
                              (mid_x + offset, mid_y), (end_x, end_y), 2)

            for sub in branch['sub_branches']:
                sub_end_x = end_x + math.cos(sub['angle']) * sub['length']
                sub_end_y = end_y + math.sin(sub['angle']) * sub['length']

                pygame.draw.line(screen, (color_r - 10, color_g - 10, color_b - 10),
                              (end_x, end_y), (sub_end_x, sub_end_y), sub['thickness'])

                pygame.draw.circle(screen, (color_r - 20, color_g - 20, color_b - 20),
                                 (int(sub_end_x), int(sub_end_y)), 3)

            pygame.draw.circle(screen, (color_r - 15, color_g - 15, color_b - 15),
    ...
    def draw(self):
            ...
            if not self.is_on_path(puddle['x'], puddle['y']):
                ...
                pygame.draw.ellipse(screen, COLORS['water_dark'],
                                  (puddle['x'] - puddle['width']//2,
                                   puddle['y'] - puddle['height']//2 + wave_offset,
                                   puddle['width'], puddle['height']))

                shimmer_width = puddle['width'] * 0.6
                shimmer_height = puddle['height'] * 0.4
                pygame.draw.ellipse(screen, COLORS['water_mid'],
                                  (puddle['x'] - shimmer_width//2,
                                   puddle['y'] - puddle['height']//2 + wave_offset - 5,
                                   shimmer_width, shimmer_height))
        ...
        for shimmer in self.shimmers:
            if not self.is_on_path(shimmer['x'], shimmer['y']):
                ...
                if highlight_size > 0:
                    pygame.draw.circle(screen, (min(255, brightness + 30), min(255, brightness + 50), 255),
                                     (int(shimmer_x), int(shimmer_y)), highlight_size)
...
class EnhancedRainParticle:
    ...
    def draw(self):
        ...
        pygame.draw.line(screen, color,
                       (self.x, self.y), (self.x + 4, self.y + self.length), self.thickness)

        if self.thickness >= 2:
            pygame.draw.line(screen, (200, 210, 230),
                           (self.x + 1, self.y + 2), (self.x + 2, self.y + 8), 1)

class EnhancedRainSystem:
    def __init__(self):
        self.particles = []
        self.splashes = []
        self.init_particles()
    ...
    def draw(self):
        ...
        for reed in self.reeds:
            if not self.is_on_path(reed['x'], reed['y']):
                ...
                pygame.draw.line(screen, (80, 90, 70),
                               (reed['x'], reed['y']),
                               (reed['x'] + sway, reed['y'] - reed['height']), 3)
        ...
        for tree in self.dead_trees:
            if not self.is_on_path(tree['x'], tree['y']):
                pygame.draw.rect(screen, COLORS['dead_tree'],
                               (tree['x'] - 8, tree['y'] - tree['trunk_height'], 16, tree['trunk_height']))
                for i in range(tree['branch_count']):
                    ...
                    pygame.draw.line(screen, COLORS['dead_tree'], (tree['x'], tree['y'] - tree['trunk_height'] * 0.7),
                                   (end_x, end_y), 4)
...
class Tower:
    ...
    def draw(self):
        ...
        if self.config['shape'] == 'triangle':
            ...
            pts = [(x + size*math.cos(i*60*math.pi/180-90*math.pi/180),
                   y + size*math.sin(i*60*math.pi/180-90*math.pi/180)) for i in range(6)]
        elif self.config['shape'] == 'diamond':
            pts = [(x, y - size), (x + size, y), (x, y + size), (x - size, y)]
        elif self.config['shape'] == 'star':
            pts = [(x + size*math.cos(i*72*math.pi/180-90*math.pi/180),
                   y + size*math.sin(i*72*math.pi/180-90*math.pi/180)) for i in range(5)]
        elif self.config['shape'] == 'octagon':
            pts = [(x + size*math.cos(i*45*math.pi/180-22.5*math.pi/180),
                   y + size*math.sin(i*45*math.pi/180-22.5*math.pi/180)) for i in range(8)]
        else:
            pts = [(x, y - size), (x + size, y), (x, y + size), (x - size, y)]
...
class Enemy:
    ...
    def draw_slime(self, x, y, size):
        ...
        pygame.draw.ellipse(screen, (self.color[0]+20, self.color[1]+30, self.color[2]+20),
