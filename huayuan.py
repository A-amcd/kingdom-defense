...
COLORS = {
    'sky_poison': (80, 90, 70),
    'sky_dark': (50, 55, 45),
    'fog_toxic': (100, 120, 80),
    'ground_corrupt': (60, 50, 40),
    'ground_dark': (40, 35, 30),
    'toxic_green': (50, 180, 50),
    'toxic_dark': (30, 100, 30),
    'poison_purple': (120, 60, 140),
    'poison_pink': (180, 100, 150),
    'bubble_toxic': (100, 200, 80),
    'dead_branch': (60, 50, 40),
    'dead_leaf': (80, 70, 50),
    'corrupt_flower': (140, 80, 120),
    'path_stone': (100, 90, 80),
    'path_dark': (70, 60, 50),
    'grid': (80, 90, 70, 30),
    'text': (180, 180, 160),
    'gold': (255, 215, 0),
...
DIALOGUE_CONFIG = {
    ...
    'name_color': (180, 160, 200),
    'heart_color': (150, 100, 180),
    'font_size': 24,
    'box_padding': 20,
    'corner_size': 8,
}
...
ENEMY_CONFIG = {
    'spirit': {'name': '枯萎灵', 'health': 90, 'speed': 1.8, 'reward': 30, 'color': (120, 100, 120), 'eye_color': (200, 80, 120)},
    'mushroom': {'name': '毒蘑菇', 'health': 150, 'speed': 0.3, 'reward': 25, 'color': (140, 80, 100), 'eye_color': (80, 200, 80)},
    'beetle': {'name': '腐蚀甲虫', 'health': 60, 'speed': 3.2, 'reward': 35, 'color': (80, 70, 90), 'eye_color': (200, 200, 80)},
    'boss': {'name': '污染之主', 'health': 1500, 'speed': 0.5, 'reward': 400, 'color': (80, 60, 80), 'eye_color': (150, 255, 100)},
}

LEVEL_CONFIG = {
    'level_4': {'name': '污染花园', 'waves': 20, 'start_gold': 650, 'lives': 28, 'theme': 'corrupt'},
}

PATH_POINTS = [
    (0, 5), (3, 5), (3, 9), (8, 9), (8, 4), (13, 4),
    (13, 11), (19, 11), (19, 6), (25, 6), (25, 14), (31, 14)
]

screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
pygame.display.set_caption("鸿蒙防线 - 污染花园")
...
class ToxicBubbleSystem:
    def __init__(self, bubble_count=50):
    ...
    def init_bubbles(self):
        for _ in range(self.bubble_count):
            self.bubbles.append({
                ...
                'phase_speed': random.uniform(0.02, 0.05),
                'opacity': random.randint(100, 180)
            })

    def update(self):
        for bubble in self.bubbles:
            bubble['phase'] += bubble['phase_speed']
            bubble['y'] -= bubble['speed']
            bubble['x'] += math.sin(bubble['phase']) * 0.3

            if bubble['y'] < SCREEN_HEIGHT // 2 - 50:
                bubble['y'] = random.randint(SCREEN_HEIGHT // 2 + 50, SCREEN_HEIGHT + 20)
                bubble['x'] = random.randint(0, SCREEN_WIDTH)

    def draw(self):
        for bubble in self.bubbles:
            opacity = int(bubble['opacity'] * (0.5 + 0.5 * math.sin(bubble['phase'])))
            color = (*COLORS['bubble_toxic'], opacity)
            surf = pygame.Surface((int(bubble['size'] * 4), int(bubble['size'] * 4)), pygame.SRCALPHA)
            pygame.draw.circle(surf, color, (int(bubble['size'] * 2), int(bubble['size'] * 2)), int(bubble['size']))
            screen.blit(surf, (int(bubble['x'] - bubble['size'] * 2), int(bubble['y'] - bubble['size'] * 2)))

class PoisonFogSystem:
    def __init__(self, particle_count=30):
        self.particles = []
        for _ in range(particle_count):
            self.particles.append({
                'x': random.randint(0, SCREEN_WIDTH),
                'y': random.randint(SCREEN_HEIGHT // 3, SCREEN_HEIGHT),
    ...
    def update(self):
        for p in self.particles:
            p['x'] += p['speed']
            if p['x'] > SCREEN_WIDTH + p['size']:
                p['x'] = -p['size']
                p['y'] = random.randint(SCREEN_HEIGHT // 3, SCREEN_HEIGHT)

    def draw(self):
        for p in self.particles:
            surf = pygame.Surface((p['size'] * 2, p['size'] * 2), pygame.SRCALPHA)
            pygame.draw.circle(surf, (*COLORS['fog_toxic'], p['opacity']),
                             (p['size'], p['size']), p['size'])
            screen.blit(surf, (int(p['x'] - p['size']), int(p['y'] - p['size'])))

class DeadCherryTree:
    def __init__(self):
    ...
    def generate_branches(self):
        ...
        for _ in range(random.randint(10, 18)):
            self.dead_flowers.append({
                'x': random.randint(-60, 60),
                'y': random.randint(-80, -20),
                'size': random.randint(6, 12),
                'color': random.choice([COLORS['corrupt_flower'], COLORS['poison_purple'], (100, 60, 80)])
            })

    def draw(self):
        pygame.draw.rect(screen, COLORS['dead_branch'],
        ...
        for branch in self.branches:
            ...
            pygame.draw.line(screen, COLORS['dead_branch'], (bx, by), (ex, ey), 4)

            for sub in branch['subs']:
                sx = ex + math.cos(sub['angle']) * sub['length']
                sy = ey + math.sin(sub['angle']) * sub['length']
                pygame.draw.line(screen, COLORS['dead_branch'], (ex, ey), (sx, sy), 2)

        for flower in self.dead_flowers:
            pygame.draw.circle(screen, flower['color'],
                             (self.x + flower['x'], self.y - self.trunk_height + flower['y']),
                             flower['size'])
            pygame.draw.circle(screen, COLORS['poison_purple'],
                             (self.x + flower['x'] - 2, self.y - self.trunk_height + flower['y'] - 2),
                             flower['size'] // 2)

class ToxicPuddle:
    def __init__(self):
        self.x = random.randint(50, SCREEN_WIDTH - 50)
        self.y = random.randint(SCREEN_HEIGHT // 2 + 50, SCREEN_HEIGHT - 50)
        self.width = random.randint(60, 120)
        self.height = random.randint(20, 40)
        self.bubbles = []
        for _ in range(random.randint(3, 6)):
            self.bubbles.append({
                'offset_x': random.uniform(-self.width // 3, self.width // 3),
    ...
    def draw(self, time):
        pygame.draw.ellipse(screen, COLORS['toxic_dark'],
                          (self.x - self.width // 2, self.y - self.height // 2, self.width, self.height))
        pygame.draw.ellipse(screen, COLORS['toxic_green'],
                          (self.x - self.width // 2 + 5, self.y - self.height // 2 + 3, self.width - 10, self.height - 6))

        for bubble in self.bubbles:
            bubble['phase'] += bubble['speed']
            bx = self.x + bubble['offset_x'] + math.sin(bubble['phase']) * 5
            by = self.y + bubble['offset_y'] + math.cos(bubble['phase'] * 0.7) * 3
            size = int(bubble['size'] * (0.5 + 0.5 * math.sin(bubble['phase'])))
            if size > 0:
                pygame.draw.circle(screen, (150, 255, 120), (int(bx), int(by)), size)

class CorruptGardenBackground:
    def __init__(self):
        self.background_surface = None
        self.trees = []
        self.puddles = []
        self.fog = PoisonFogSystem(25)
        self.bubbles = ToxicBubbleSystem(40)
        self.generate_elements()

    def generate_elements(self):
        for _ in range(5):
            self.trees.append(DeadCherryTree())
    ...
    def draw(self):
        ...
        for y in range(int(SCREEN_HEIGHT * 0.5), SCREEN_HEIGHT):
            fog_alpha = int(((y - SCREEN_HEIGHT * 0.5) / (SCREEN_HEIGHT * 0.5)) * 50)
            fog_surface.fill((*COLORS['fog_toxic'], fog_alpha), (0, y, SCREEN_WIDTH, 1))
...
background = CorruptGardenBackground()
...
class IntroSequence:
    def __init__(self):
        ...
        self.text_index = 0
        self.wait_timer = 0
        self.bubbles = ToxicBubbleSystem(30)
        self.fog = PoisonFogSystem(15)
        self.background_surface = None
    ...
    def init_intro(self):
        self.dialogues = [
            ...
            {"text": "所有的生命都被腐蚀、扭曲...", "speaker": "向导", "color": (180, 160, 200)},
            {"text": "变异生物从废墟中苏醒！", "speaker": "哨兵", "color": (200, 150, 180)},
            {"text": "污染之主正在苏醒！", "speaker": "哨兵", "color": (200, 150, 180)},
            {"text": "我们必须阻止这场灾难！", "speaker": "英雄", "color": (180, 255, 200)},
            {"text": "绝不能让它逃出这片废墟！", "speaker": "英雄", "color": (180, 255, 200)},
    ...
    def draw_intro_text(self):
        ...
        pygame.draw.rect(screen, COLORS['poison_purple'], (box_x, box_y, box_width, box_height), 4)
        ...
        for i in range(4):
            ...
            pygame.draw.circle(screen, COLORS['poison_purple'], (corner_x, corner_y), 8, 4)
        ...
        if self.text_index >= len(self.target_text):
            ...
            heart_x = box_x + 20
            pygame.draw.polygon(screen, COLORS['poison_purple'], [
                (heart_x, heart_y - 10),
                (heart_x - 10, heart_y),
                (heart_x, heart_y + 8),
                (heart_x + 10, heart_y)
...
class Tower:
    ...
    def draw(self):
        ...
        pygame.draw.rect(screen, COLORS['ground_dark'], (x - size - 4, y + size // 2, size * 2 + 8, size // 2 + 4))
        pygame.draw.rect(screen, COLORS['dead_branch'], (x - size - 2, y + size // 2 + 2, size * 2 + 4, size // 2))
...
class Enemy:
    def __init__(self, enemy_type, path_index=0, wave_num=1):
        ...
        wave_multiplier = 1 + (wave_num - 1) * 0.15
        ...
        self.size = min(self.size, 24)
        self.anim_time = 0
        self.float_offset = 0
        self.poison_particles = []

        base_color = self.config['color']
        corrupt_factor = min(wave_num * 0.04, 0.4)
        self.color = (
            max(0, int(base_color[0] * (1 - corrupt_factor * 0.3))),
            max(0, int(base_color[1] * (1 - corrupt_factor * 0.5))),
            min(255, int(base_color[2] + corrupt_factor * 50)),
        )
    ...
    def update(self, current_time):
        ...
        for p in self.poison_particles[:]:
            p['life'] -= 1
            if p['life'] <= 0:
                self.poison_particles.remove(p)
    ...
    def draw(self):
        for p in self.poison_particles:
            ...
            pygame.draw.circle(surf, (*COLORS['toxic_green'], alpha), (int(p['size']), int(p['size'])), int(p['size']))
        ...
        if self.type == 'spirit':
            wing_flap = math.sin(pygame.time.get_ticks() * 0.006) * 0.25
            points = [
                (x, y - size),
                (x - size * 1.3, y + size * wing_flap),
                (x, y + size // 2),
                (x + size * 1.3, y + size * wing_flap)
            ]
            pygame.draw.polygon(screen, self.color, points)
            pygame.draw.polygon(screen, (150, 100, 140, 150), points, 2)
            pygame.draw.circle(screen, (180, 150, 160), (x - 3, y - 3), 3)
            pygame.draw.circle(screen, (180, 150, 160), (x + 3, y - 3), 3)
            pygame.draw.circle(screen, self.config['eye_color'], (x - 3, y - 3), 2)
            pygame.draw.circle(screen, self.config['eye_color'], (x + 3, y - 3), 2)
        elif self.type == 'mushroom':
            stem_width = size // 2
            stem_height = size * 1.2
            pygame.draw.rect(screen, (160, 140, 120), (x - stem_width // 2, y - stem_height // 2, stem_width, stem_height))
            pygame.draw.ellipse(screen, self.color, (x - size, y - size - stem_height // 2, size * 2, size * 1.5))
            pygame.draw.ellipse(screen, (180, 120, 140), (x - size + 5, y - size - stem_height // 2 + 5, size * 2 - 10, size * 1.5 - 10))
            for i in range(3):
                spot_x = x + (i - 1) * size * 0.5
                spot_y = y - size // 2 - stem_height // 2
                pygame.draw.circle(screen, (200, 220, 180), (int(spot_x), int(spot_y)), 3)
            pygame.draw.circle(screen, (180, 150, 160), (x - 4, y - 5), 4)
            pygame.draw.circle(screen, (180, 150, 160), (x + 4, y - 5), 4)
            pygame.draw.circle(screen, self.config['eye_color'], (x - 4, y - 5), 2)
            pygame.draw.circle(screen, self.config['eye_color'], (x + 4, y - 5), 2)
        elif self.type == 'beetle':
            shell_angle = math.sin(pygame.time.get_ticks() * 0.008) * 0.1
            pygame.draw.ellipse(screen, self.color, (x - size // 2, y - size // 2, size, size))
            pygame.draw.ellipse(screen, (100, 90, 110), (x - size // 2 + 3, y - size // 2 + 3, size - 6, size - 6))
            points = [
                (x - size // 2 - 5, y - 3),
                (x - size // 2 - 10, y - 8),
                (x - size // 2 - 5, y - 5)
            ]
            pygame.draw.polygon(screen, (60, 50, 70), points)
            points = [
                (x + size // 2 + 5, y - 3),
                (x + size // 2 + 10, y - 8),
                (x + size // 2 + 5, y - 5)
            ...
            pygame.draw.polygon(screen, (60, 50, 70), points)
            pygame.draw.circle(screen, (200, 200, 100), (x - 3, y - 3), 2)
            pygame.draw.circle(screen, (200, 200, 100), (x + 3, y - 3), 2)
            pygame.draw.circle(screen, self.config['eye_color'], (x - 3, y - 3), 1)
            pygame.draw.circle(screen, self.config['eye_color'], (x + 3, y - 3), 1)
        elif self.type == 'boss':
            pygame.draw.polygon(screen, (60, 40, 60), [(x, y - size - 25), (x - size * 1.6, y + size // 2), (x + size * 1.6, y + size // 2)])
            pygame.draw.circle(screen, self.color, (x, y - size // 2), size // 2 + 10)
            pygame.draw.polygon(screen, (80, 50, 80), [(x - size // 2, y - size // 2), (x - size * 1.6, y - size * 2), (x - size // 3, y - size // 3)])
            pygame.draw.polygon(screen, (80, 50, 80), [(x + size // 2, y - size // 2), (x + size * 1.6, y - size * 2), (x + size // 3, y - size // 3)])

            for i in range(4):
                angle = i * (math.pi / 2) + pygame.time.get_ticks() * 0.002
                orbit_x = x + math.cos(angle) * (size + 15)
                orbit_y = y - size // 2 + math.sin(angle) * (size + 15)
                pygame.draw.circle(screen, COLORS['toxic_green'], (int(orbit_x), int(orbit_y)), 6)
                pygame.draw.circle(screen, (200, 255, 150), (int(orbit_x), int(orbit_y)), 3)

            pygame.draw.circle(screen, self.config['eye_color'], (x - 7, y - size // 2 - 4), 7)
            pygame.draw.circle(screen, self.config['eye_color'], (x + 7, y - size // 2 - 4), 7)
            pygame.draw.circle(screen, (255, 255, 255), (x - 7, y - size // 2 - 4), 3)
            pygame.draw.circle(screen, (255, 255, 255), (x + 7, y - size // 2 - 4), 3)

            for i in range(5):
                bx = x - size // 2 + i * size // 2
                by = y + size // 2 + 5
                pygame.draw.circle(screen, COLORS['toxic_green'], (int(bx), int(by)), 3)

        bar_width = 30
        bar_height = 4
        pygame.draw.rect(screen, (30, 30, 30), (x - bar_width // 2, y - size - 12, bar_width, bar_height))
        health_ratio = max(0, self.health / self.max_health)
        bar_color = (100, 200, 100) if health_ratio > 0.5 else (200, 200, 80) if health_ratio > 0.25 else (200, 80, 80)
        pygame.draw.rect(screen, bar_color, (x - bar_width // 2, y - size - 12, bar_width * health_ratio, bar_height))
...
class Game:
    ...
    def draw_path(self):
        ...
        sx, sy = PATH_POINTS[0]
        ex, ey = PATH_POINTS[-1]
        pygame.draw.circle(screen, COLORS['toxic_green'], (sx * GRID_SIZE + GRID_SIZE // 2, sy * GRID_SIZE + GRID_SIZE // 2), 15)
        pygame.draw.circle(screen, (150, 255, 150), (sx * GRID_SIZE + GRID_SIZE // 2, sy * GRID_SIZE + GRID_SIZE // 2), 15, 3)
        pygame.draw.circle(screen, COLORS['poison_purple'], (ex * GRID_SIZE + GRID_SIZE // 2, ey * GRID_SIZE + GRID_SIZE // 2), 15)
        pygame.draw.circle(screen, (200, 150, 200), (ex * GRID_SIZE + GRID_SIZE // 2, ey * GRID_SIZE + GRID_SIZE // 2), 15, 3)
    ...
    def update_wave(self, current_time):
        ...
        wave = level['waves']
        base_enemies = 7 + self.wave_index * 2
        delay = max(450, 1300 - self.wave_index * 35)
        ...
        if self.enemy_index < base_enemies:
            if current_time - self.last_spawn_time >= delay:
                if self.wave_index >= wave * 0.7:
                    enemy_type = 'mushroom' if self.wave_index % 3 == 0 else 'beetle'
                elif self.wave_index >= wave * 0.4:
                    enemy_type = 'beetle' if self.wave_index % 2 == 0 else 'spirit'
                else:
                    enemy_type = 'spirit'
    ...
    def draw(self):
        ...
        if self.game_state == 'preparing':
            ...
            text = large_font.render("污染花园", True, COLORS['poison_purple'])
            screen.blit(text, text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 80)))
            text = small_font.render("被毒素侵蚀的废弃花园，变异生物横行", True, (160, 140, 160))
            ...
            pygame.draw.rect(screen, (100, 80, 100), btn)
            pygame.draw.rect(screen, COLORS['poison_purple'], btn, 3)
        ...
        if self.game_state == 'victory':
            ...
            surf.fill((0, 0, 0, 160))
            screen.blit(surf, (0, 0))
            text = large_font.render("胜利！", True, COLORS['toxic_green'])
            screen.blit(text, text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 50)))
            text = font.render(f"分数: {self.score}", True, COLORS['poison_purple'])
    ...
    def draw_ui(self):
        pygame.draw.rect(screen, (70, 60, 70), (0, 0, SCREEN_WIDTH, 50))

        pygame.draw.rect(screen, (90, 80, 90), (10, 8, 120, 34))
        text = font.render(f"{self.gold}", True, COLORS['gold'])
        screen.blit(text, text.get_rect(center=(70, 25)))

        pygame.draw.rect(screen, (90, 80, 90), (140, 8, 120, 34))
        text = font.render(f"{self.lives}", True, COLORS['health'])
        screen.blit(text, text.get_rect(center=(200, 25)))

        pygame.draw.rect(screen, (90, 80, 90), (270, 8, 120, 34))
        level = LEVEL_CONFIG[self.level]
        ...
        pygame.draw.rect(screen, (70, 60, 70), (panel_x, 60, 180, 340))
        ...
        for tower_type, config in TOWER_CONFIG.items():
            ...
            if self.selected_tower_type == tower_type:
                pygame.draw.rect(screen, (90, 80, 90), bg)
                pygame.draw.rect(screen, COLORS['poison_purple'], bg, 3)
