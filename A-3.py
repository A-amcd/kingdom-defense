...
pygame.display.set_caption("鸿蒙防线 - 剧情版")
...
class CharacterPortrait:
    ...
    def show(self, side='left'):
        self.is_active = True
        self.x_offset = -300 if side == 'left' else 300
        self.target_x_offset = 0
        self.alpha = 0
        self.target_alpha = 255
        
    def hide(self):
        self.target_x_offset = -300 if self.x_offset < 0 else 300
        self.target_alpha = 0
    ...
    def draw(self, surface, x, y):
        ...
        glow_surface = pygame.Surface((16 * self.scale + 20, 16 * self.scale + 20), pygame.SRCALPHA)
        glow_alpha = int(self.alpha * 0.3)
        pygame.draw.rect(glow_surface, (255, 215, 0, glow_alpha), 
                        (0, 0, 16 * self.scale + 20, 16 * self.scale + 20), 
                        border_radius=20)
    ...
    def get_current_frame(self):
        if self.character_id == 'hero':
            return HERO_FRAMES[self.animation_frame]
        elif self.character_id == 'guide':
            return GUIDE_FRAMES[self.animation_frame]
        elif self.character_id == 'sentinel':
            return SENTINEL_FRAMES[self.animation_frame]
        return HERO_FRAMES[0]
...
class IntroSequence:
    ...
    def get_dialogues(self, theme):
        dialogues_map = {
            'plain': [
                ...
                {"text": "建立防御塔，阻止敌人的进攻！", "speaker": "英雄", "color": (255, 215, 0)},
    ...
    def draw(self):
        ...
        self.character.draw(screen, 100, SCREEN_HEIGHT - 250)
    ...
    def draw_intro_text(self):
        ...
        box_width = min(SCREEN_WIDTH - 100, 900)
        box_height = 150
        ...
        if speaker:
            ...
            name_rect = name_surface.get_rect(center=(box_x + 100, box_y - 20))
        ...
        text_x, text_y = box_x + 30, box_y + 30
        ...
        for i, line_text in enumerate(lines[:4]):
            if line_text:
                screen.blit(font.render(line_text, True, color), (text_x, text_y + i * 35))

        if self.text_index >= len(self.target_text):
            heart_x, heart_y = box_x + 30, box_y + box_height - 40
            pygame.draw.polygon(screen, color, [(heart_x, heart_y - 12), (heart_x - 12, heart_y), (heart_x, heart_y + 10), (heart_x + 12, heart_y)])
...
class Game:
    ...
    def draw_level_select(self):
        ...
        for i, (level_id, config) in enumerate(level_list):
            ...
            icon_drawer = icon_drawers.get(config.get('icon_theme', 'plain'), icon_drawers['plain'])
